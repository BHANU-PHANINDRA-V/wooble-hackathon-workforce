...
export default function WorkerDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl sm:text-3xl font-black text-white">Rahul Kumar</h1>
              <ShieldCheck className="w-6 h-6 text-emerald-400 fill-emerald-950" />
            </div>
            <p className="text-sm font-semibold text-amber-400">Industrial & Commercial Electrician (6 Yrs Exp)</p>
            <div className="flex items-center gap-3 text-xs text-slate-300 mt-1">
              <span className="flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-slate-400" /> Cherlapally, Hyderabad
              </span>
              <span>•</span>
      ...
      {/* Profile Strength & Insights */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-slate-500 uppercase">Profile Strength</span>
            <span className="text-sm font-black text-blue-700">95% Complete</span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-2.5 mb-4">
            <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: "95%" }} />
          </div>
          <p className="text-xs text-slate-600">
            ✓ Identity verified, 4 skills certified, 2 trade licenses uploaded.
          </p>
        </div>
        ...
        <div className="md:col-span-2 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl p-6 flex flex-col justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-extrabold text-emerald-950">High Demand in Hyderabad & Vijayawada</h3>
              <p className="text-xs text-emerald-800 mt-1 leading-relaxed">
                Your profile ranks in the <strong>top 4%</strong> of industrial electricians in Telangana. 8 active employers are hiring for substation & solar projects this week.
              </p>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Matched based on verified skills, experience, and 20 km radius</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {jobs.slice(0, 4).map((job, idx) => {
              const matchScore = idx === 0 ? 94 : idx === 1 ? 91 : idx === 2 ? 88 : 85;
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-semibold">{job.company?.name || "Tata Projects"}</p>
                      </div>
                      ...
                      <button
                        onClick={() =>
                          setSelectedJobForExplainer({
                            ...
                            reasons: [
                              "✓ Verified Industrial Electrical Wiring matches 100% of required job criteria",
                              "✓ 6 Years experience exceeds the 4+ year requirement",
                              "✓ Located 8.4 km away (within your 20 km radius)",
                              "✓ Available immediately for Day Shift",
